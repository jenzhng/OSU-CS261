...
/* Name: Jenny Zhong
* Date: 10/19/22
* Solution description: This program is used to create and update a binary tree
*/
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	if (cur == 0)
	{
		...
		newNode->left = 0;			//Set left to zero
		newNode->right = 0;			//Set right to zero

		return newNode;
	}

	if (compare(val, cur->val) == -1)				//Check if left is greater then right
		cur->left = _addNode(cur->left, val);
	else											//Else right is greater then left
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	int path = 0;
	struct Node *transverse = tree->root;
	assert(transverse != 0);

	while (transverse != 0)
	{
		path = compare(val, transverse->val);
...
TYPE _leftMost(struct Node *cur)
{
	...
	if (cur->left == 0)						//Check if left is zero
		return cur->val;
	else
		return _leftMost(cur->left);		//Return until left has no value
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	if (cur->left != 0)							
	{
		cur->left = _removeLeftMost(cur->left);		//set left to remove left most
		return cur;
	}
	else											//(cur->left == 0)
	{
		...
		struct Node *tempNode = cur->right;			//tempNode to return right
		free(cur);									//release
		return tempNode;							//return right
	}
}
/*
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	if (compare(val, cur->val) == 0)					//Compare cur node is at value
	{
		if (cur->right == 0)							
		{
			tempNode = cur->left;						//set left
			free(cur);									//free left
			return tempNode;							//return left
		}
		else
		{
			cur->val = _leftMost(cur->right);			//get value of left most cur right
			cur->right = _removeLeftMost(cur->right);	//set to remove right
		}
	}
	else if (compare(val, cur->val) < 0)				
		cur->left = _removeNode(cur->left, val);		//set to remove left
	else
		cur->right = _removeNode(cur->right, val);		//set to remove right
