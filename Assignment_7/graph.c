...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	dequePushFront(stack, source);				//Add vs to a stack

	while (dequeIsEmpty(stack) != 1)			//While stack is not empty
	{
		Vertex* current = dequeFront(stack);	//Get last vertex v from stack
		dequePopFront(stack);					//and remove(pop) last vertex v from stack
		current->isVisited = 1;

		if (current == destination)				//v is known to be reachable, discard
			return 1;

		for (int i = 0; i < current->numNeighbors; i++)
		{														//vj is not is set of reachable vertices,
			if ((current->neighbors[i]->isVisited) == 0)		//For all neighbors, vj, of v
				dequePushFront(stack, current->neighbors[i]);	//add to stack
		}
	}
	dequeDelete(stack);			//remove stack
	return 0;
}
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	dequePushBack(queue, source);				//Add vs to a queue

	while (dequeIsEmpty(queue) != 1)			//While queue is not empty
	{
		Vertex* current = dequeBack(queue);		//Get vertex v from queue
		dequePopBack(queue);					//remove(pop) with shortest distance from priority queue
		current->isVisited = 1;
		...
		{														//vj is not is set of reachable vertices, 
			if ((current->neighbors[i]->isVisited) == 0)		//For all neighbors, vj, of v
				dequePushBack(queue, current->neighbors[i]);	//add to queue
		}
	}
	dequeDelete(queue);			//remove queue
	return 0;
}
